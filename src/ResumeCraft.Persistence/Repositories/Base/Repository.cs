...
using ResumeCraft.Persistence.Repositories.Base.Skeleton;
using System.Data;
...
namespace ResumeCraft.Persistence.Repositories.Base
{
    public abstract class Repository<TEntity, TKey> : IRepository<TEntity, TKey>, IRepositoryBase<TEntity, TKey>
        ...
        where TKey : IComparable
    {
        ...
        public async Task<TEntity> GetByIncludingAsync(
            Expression<Func<TEntity, bool>> filter,
            Expression<Func<TEntity, object>> navigationPropertyPath
        )
        {
            return (await _dbSet.Include(navigationPropertyPath).Where(filter).FirstOrDefaultAsync())!;
        }
        ...
        )
        {
            ...
            if (outParameters != null)
            {
                ...
                {
                    command.Parameters.Add(CreateOutputParameter(item.Key, item.Value));
                }
            }
        ...
        private object ChangeType(Type propertyType, object itemValue)
        {
            ...
            return itemValue is decimal && propertyType == typeof(double) ? Convert.ToDouble(itemValue) : itemValue;
        }
        ...
        private IDictionary<string, object> CopyOutParams(DbCommand command, IDictionary<string, Type> outParameters)
        {
