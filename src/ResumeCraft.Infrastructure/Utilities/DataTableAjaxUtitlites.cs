...
namespace ResumeCraft.Infrastructure.Utilities
{
    public class DataTableAjaxUtitlites
    {
        private HttpRequest _request;

        public DataTableAjaxUtitlites(HttpRequest request)
        {
            _request = request;
        }
        ...
        public int Start
        {
            get { return int.Parse(RequestData.Where(x => x.Key == "start").FirstOrDefault().Value!); }
        }
        public int Length
        {
            get { return int.Parse(RequestData.Where(x => x.Key == "length").FirstOrDefault().Value!); }
        }
        ...
        public string GetSortText(string[] columnNames)
        {
            ...
            if (sortText.ToString().Trim().Equals("asc") || sortText.ToString().Trim().Equals("desc"))
                return "Id asc";
